...
new_cata.write('#'+str(field[list_idx])[1:-1]+'\n')
...
for file_name in CataList:
	...
	#cata_file = open(catPath+Proj[p_index]+file_name,'r')
	sub_table = np.loadtxt(cata_file,skiprows=1,unpack=True, usecols=f_idx[list_idx])
...
write_file.write('#'+str(field[list_idx])[1:-1]+'\n')
