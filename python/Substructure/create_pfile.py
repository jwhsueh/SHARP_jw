...
dm_x,dm_y,dm_z = dm_x-np.min(dm_x),dm_y-np.min(dm_y),dm_z-np.min(dm_z)
gas_x,gas_y,gas_z = gas_x-np.min(gas_x),gas_y-np.min(gas_y),gas_z-np.min(gas_z)
st_x,st_y,st_z = st_x-np.min(st_x),st_y-np.min(st_y),st_z-np.min(st_z)
...
out_file = open('../../data/illustris_1/snapshot_'+str(snapNum)+'_particle/particle_'+str(GalaxyID)+'_st.dat','w')

out_file.write('# nparticles '+str(st_x.size)+'\n')
