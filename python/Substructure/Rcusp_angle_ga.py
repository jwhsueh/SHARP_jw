...
outpath='/Volumes/sting_1/data/'

list_file = listpath+'/snap99_tri2_Rcusp.txt'
size_file = listpath+'/snap99_tri2_size.txt'
...
#list_tab = ['179899_p1','179899_p1','177217_p3']
#size = np.array([4.66E-04,0.00041793736,0.00095426498])

list_tab = ['238069_p2']
size = np.array([np.degrees(9.13E-06)])
#size = np.array([0.00071305158]) # in deg
n_src = np.array([100])
...
for i in range(len(list_tab)):
	...
	filepath='/Volumes/sting_1/snap99_'+subID+'_test'
	...
	for j in range(n_src[i]):
		if (~np.in1d(j,drop_list)): # if the src is not dropped
			...
			for k in range(img_x.size):
				img_cut = image[img_y[k]-boxsize/2:img_y[k]+boxsize/2,img_x[k]-boxsize/2:img_x[k]+boxsize/2]
				...
				while (fit_out.x_mean < 1):
					...
					img_cut = image[img_y[k]-boxsize_ad/2:img_y[k]+boxsize_ad/2,img_x[k]-boxsize_ad/2:img_x[k]+boxsize_ad/2]
					fit_out = fit_g(gaus2d,grid_xad,grid_yad,img_cut)
					print boxsize_ad
			...
			rfold_k=(np.sum(dou_fk)-2.*mid_fk)/np.sum(dou_fk)
			rcusp_k=(np.sum(img_fk)-2.*mid_fk)/np.sum(img_fk)

			print rfold[0],rcusp[0],phi0,phi1,rfold_k[0],rcusp_k[0]

			rcusp_file.write(str(rfold[0])+'\t'+str(rcusp[0])+'\t'+str(phi0)+'\t'+str(phi1)+'\t'+str(rfold_k[0])+'\t'+str(rcusp_k[0])+'\n')
