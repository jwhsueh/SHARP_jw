...
SubVel = groupcat.loadSubhalos(snapshotPath,snapNum, fields = ['SubhaloVel'])
...
in_cata.write('# Galaxy ID 	theta_x		theta_y		theta_z \n')
...
for i in range(GalaxyID.size):
	...
	subhalo_st = snapshot.loadSubhalo(snapshotPath,snapNum,GalaxyID[i],'stars')
	...
	in_cata.write(str(GalaxyID[i])+'\t'+str(theta_i[i,:])[1:-1]+'\n')