...
In part 3, we present new lens models of B1555+375. The most significant difference comparing to previous modelling is a new edge-on disk component. With a bulge plus an edge-on disk, our models fairly match the radio image positions and also flux ratio anomalies, while models with only one SIE component \citep{Marlow,Xu14} fail. Radio propagation effects are ruled out in B1555+375 \citep{K03,KD04}. Our successful modelling indicates that the structure from lens galaxy itself causes the flux ratio anomalies in B1555+375. An edge-on disk with high ellipcity well explains the observed flux ratios without the presense of substructrues. This breaks the idea that in strongly lensed quasars, radio flux ratio anomalies are most likely originated from substructure perturbations. It is worth noticing not all of flux ratio anomalies in lensed quasars indicate the existence of substructures. Our discover shows a complex lens galaxy mass distribution can dominate flux ratio anomalous phenomenon in the system. Although the number of radio loud flux anomalous lensed quasars remains small, new technique using narrow-line emission will enlarge the sample and bring statistical perspective on substructures in the near future \citep{N14}. To gather correct statistics, one must fully consider non-substructure effects. Otherwise, possible ``false'' substructures detected by flux anomalies can serverly biased the data analysis. Even with real substructures presence, lens galaxy structure can still dominate the flux anomalous phenomenon and have impact on estimated mass and also other derived properties. We would like to draw attention on this non-substructure effect brought by a large, high ellipcity edge-on disk. For precise measurement on substurcture population, mass function, and any derived properties from strong lensed flux anomalous systems, careful estimation on non-substructure effects are important.
