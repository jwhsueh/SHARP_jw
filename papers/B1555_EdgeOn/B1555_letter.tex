...
\textbf{To obtain the best-fit lens model, we explore the parameter space as the following:  First, we use the best-fit model of \citet{Marlow99} as the starting point for the SIE profile. By looking at the NICMOS F160W imaging, which has the highest signal-to-noise ratio of disc among the data, we fix the centroid position as $(-0.162, -0.206)$ , ellipticity as $e=0.83$, and position angle of $\theta=8^\circ$. The total number of free parameters is 9. After obtaining the best-fit lens model from {\sc gravlens}, we run a Markov-Chain Monte-Carlo (MCMC) analysis to obtain uncertainties.}
...
%   & (SIE) & (ExpDisc)\\
\hline
$\Delta$RA	& $-0.176 \pm 0.005$		& $-0.162$ \textit{(fixed)}\\
$\Delta$Dec	& $-0.212 \pm 0.005$		& $-0.206$ \textit{(fixed)} \\
$b$ 			& $0.19 \pm 0.01$  		& ...  \\
$\kappa_0$ & ... &$0.19 \pm 0.01$\\
$e$	  		& $0.40 \pm 0.01$			& $0.83$ \textit{(fixed)} \\
$\theta$ 		& $4.4 \pm 0.6$			& $8.0$ \textit{(fixed)}	 \\
$R_s$			& ...  						& $0.21 \pm 0.01$	 \\
\hline
\label{tab:model}
\end{tabular}
