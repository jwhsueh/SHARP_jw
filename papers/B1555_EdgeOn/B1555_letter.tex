...
\textbf{Our next step is to examine the idea that if an edge-on disk can cause similar perturbations to the lens like substructures which resulting in flux ratio anomalies in merging images. In \citet{Kee98}, they explore strong lensing by spiral glaxies with SIE+ exponential disk+NFW model, corresponding to the bulge, disk, and halo respectively. An exponential disk profile best describes the disk component in spiral galaxies because it matches the light distribution of disk. We thus choose a SIE plus exponential disk model (SIE+expdisk) in our study of B1555+375, focusing on the role of an edge-on disk passing through the merging double in the lens system. }
...
\textbf{To obtain the optimal lens model, we explore the parameter space in the following way: 
...
(2) Optimal model search: We let all model parameter varies but bind the centroid position of two conponents with a strong Gaussian prior ($\sigma = 0.05 $ arcsec). Run optimization with amoeba algorithm in {\tt gravlens} interatively to reach the global minimum of $chi^2$ in paramter space.
(3) Markov Chain Monte Carlo: After obtaining the optimal lens model, we run MCMC to explore the parameter space for the model uncertainties.} 
...
\textbf{By adding an elongated exponential disk to a smooth potential model,  our lens model shows significant improvement on the flux ratios when comparing to the single SIE model, includeing the flux ratio anomaly between the merging images.}
%the 2-SIE version has a somewhat
%better performance than the SIE+expdisk model in reproducing flux
%ratios. 
The comparison of the observations and our lens modelling is
given in Figure 3 and Tables 3 \& 4.  We have not included a third component such as an external
...
\begin{table}
 \centering
% \begin{minipage}{140mm}
  \caption{Best-fit parameters of B1555+375 lens model. Positions are offsets from radio image component A measured in units of arcsec. The velocity dispersion $\sigma$ is in units of $km/s$. \textbf{$e$ is ellipticity defined as the axis ratio $q=1-e$ }and $\theta$ is the position angle measured east of north.}
  \begin{tabular}{@{}ccc}
\hline 
