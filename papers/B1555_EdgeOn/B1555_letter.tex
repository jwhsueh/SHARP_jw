...
this discrepancy, it is imperative to build up a large sample of satellites/\simona{subhaloes} in galaxies outside the Local Group.  
...
\simona{It was first suggested by Mao \& Schneider 1998 that flux ratio anomalies observed in radio loud multiple images of lensed quasar could be interpreted as the telltale of the presence of substructure in lens galaxies.}
...
\section{Discussion}
The abundance of substructure within galaxy-size halo has been an undetermined problem in structure formation studies. The discrepency between numerical simulations and observations within \simona{the} \simona{L}ocal \simona{U}niverse gives two possible arguments: an alternative structure formation model or the selection bias of local universe samples. Lensing shows a promising potential on substructure detection beyond the Local Group. The main reason is that \simona{the} gravitational perturbation from the substructure reflects the total mass, regardless it's luminous or not.
