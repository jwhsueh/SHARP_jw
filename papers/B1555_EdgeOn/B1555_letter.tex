...
\textbf{We use {\sc gravlens} to obtain the best-fit SIE+ExpDisc lens model. The best-fit parameters from \citet{Marlow99} are set as the starting point of the SIE profile. By looking at the NICMOS F160W imaging, which has the highest signal-to-noise ratio of disc among the data, we fix the centroid position as $(-0.162, -0.206)$ , ellipticity as $e=0.83$, and position angle of $\theta=8^\circ$. The total number of free parameters is 9. After obtaining the best-fit lens model, we run a Markov-Chain Monte-Carlo (MCMC) analysis to obtain uncertainties.}
...
The positions are all in agreement, and the flux ratios are consistent at the $2\sigma$ level, with excellent agreement now obtained for the two merging images A and B. The remaining differences in fluxes may due to the real lensed source being slightly extended rather than the point source that we used in our model. The model requires that the \textbf{ExpDisc component makes up about 15 per cent of the total projected mass within the Einstein radius (1.7~kpc for an assumed lens redshift of $z_l = 1$).  In this central region of an edge-on spiral galaxy, the baryonic mass can be a significant component of the total projected mass. Studies on edge-on spiral lenses, for example the SWELLS survey, show that the disc can contribute more than 20 per cent mass in the Einstein radius \citep{swells1,swells3,swells6}.} 
...
perturbation in the mass model.  \textbf{Although the constraint on disc mass from photometry is currently not avaliable due to the low signal-to-noise ratio in AO imaging and the lack of redshift information, our model indicates that the flux anomalies in B1555+375, at least, have strong link to the present of its edge-on disc. } Such
...
disc. \textbf{Some of concerns may arise that an overestimated disc component leads to a model can explain the entire anomaly. One is saying can a spiral lens with massive disc not distort the arc close to the disc. J0820+4847, one of the SWELLS samples, is an edge-on lens has an arc right across its disc. The SWELLS studies indicate it has 15 per cent of disc mass within the Einstein radius. Other SWELLS lenses have disc mass fraction range from 5 per cent to 30 per cent \citep{swells1,swells3,swells6}. These numbers tell that the disc in our best-fit model of B1555+375 still fits into the nature of general spiral lenses.}  However, it may be possible to independently constrain the mass
...
Our analysis of B1555+375 demonstrates that the choice of the macromodel used to describe the `smooth' lens galaxy mass distribution, combined with the limited information provided by the position of compact lensed images, can lead to a misinterpretation of the origin of flux-ratio anomalies in some systems. \textbf{\citet{Maller97,Kee98, Moller03} have shown that a disc with high inclination angle can significantly affects the lensing cross-section. While the current sample of lensed quasars with flux ratio anomalies is fairly small, few systems with edge-on disc is sufficient to bring a bias into the substructure analysis. Moreover, near-infrared AO imaging of
other lenses in the SHARP sample shows that B1555+375 is not the only
system that has a significant disc component detection close to merging images among the current sample of flux ratio anomalous systems; these gravitational lenses will be investigated in future
work, but the images alone suggest that other systems may have similar
links between flux-ratio anomalies and disc components. This bias brought by edge-on disc may be avoided by post-selection as the sample size grows in the future. Therefore, it is critical to obtain high-resolution imaging like SHARP of target lenses to detect `sharp' mass features such as edge-on disks or even massive luminous satellites.} 
