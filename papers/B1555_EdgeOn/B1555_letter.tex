...
The next step is to test if a physically plausible edge-on disc can cause a perturbation in the strong lensing mass model that produces the observed flux ratio anomaly of  merging images A and B. \textbf{Our model contains a SIE plus an exponential disc component (SIE+ExpDisc) due to that  exponential disc profile matches the light distribution of the disc component in spiral galaxies \citep{Kee98}.} The free parameters of this model are the SIE Einstein radius ($b$), centroid position, ellipticity ($e = 1 - q$), position angle ($\theta$), the exponential disc intrinsic central density ($\kappa_0$) and scale length ($R_s$).\textbf{ A detailed definition of these mass profiles is described by \citet{Kee01} eq. (73) \& (74) .} The source position adds two additional model parameters. 
...
\textbf{While} investigations of edge-on spiral lenses, for example from the SWELLS survey, show that the disc mass fraction within the Einstein radius ranges from 5 to 30 per cent\citep{swells1,swells3,swells6}.
