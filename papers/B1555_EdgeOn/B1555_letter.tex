...
shown in the B0128 system [REF??, In Dandan's paper?].  However, these propagation effects
can be disentangled from substructure lensing because they have well
known dependencies on wavelength. [COMMENTS ABOUT B1555 AND OTHER
RADIO-LOUD SYSTEMS, from K03 paper].    
...
Recent studies on numerical simulation support our discover on B1555+375. In \citet{Xu14}, they found that substructures cannot expalin all radio flux anomalous phenomena in radio loud lensed systems. By adding subhaloes on to those systems, only one out of six lens systems reproduces the flux anomalies. The rest of them are unlikely (only few percent likelihood) to have substructure originated flux anomalies, including our target B1555+375. \citet{Xu14} conclude the failure in predicting fluxes of those lens systems is due to improper lens models. Their results are consistent with the SHARP $K^{\prime}$ band observations, which in B1555+375, an edge-on disk lies cross the radio merging double. Here we would like to draw attention to the possible impacts on substructure searching from the enge-on disk orginated flux anomalies. In SHARP observations, we find B1555+375 is not the only system having edge-on disk detection in high resolution near-infrared imaging (see Fig 4). It is a sign that researchers should be careful about the flux anomalies fraction in lensed quasars. It may be worth considering an upper limit of flux anomalies fraciton from observation results to avoid over estimate the number of substructure detections. Although the number of radio loud flux anomalous lensed quasars remains small, a new technique using narrow-line emissions will enlarge the sample and bring statistical perspective on substructures in the near future \citep{N14}. For further analysis on substurcture population, mass function etc., it is always important to rule out other non-substructure effects. 
