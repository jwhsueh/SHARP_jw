...
src_file = '/Volumes/sting_1/snap99_173853/173853_p1_src.dat'
src_rad = np.loadtxt(src_file)
src_px,src_py= np.degrees(src_rad[:,0])*3600,np.degrees(src_rad[:,1])*3600
src_g0 = np.array([1.111263e-01 , 1.105592e-01])
src_p0 = np.array([src_px[0],src_py[0]])

src_tx,src_ty = (src_px-src_p0[0])+src_g0[0], (src_py-src_p0[1])+src_g0[1]

# SIE mag output
sie_out = open('/Volumes/sting_1/data/Rcusp_c/173853_p1_64_Rcusp_sie.txt','w')

for i in range(src_tx.size):
	findimg_file = open('173853_findimg.input','w')
	print src_tx[i],src_ty[i]
	findimg_file.write('gridmode 1\nset ngrid1=25\nset ngrid2=25\nset maxlev=3\n')
	findimg_file.write('startup 1 1\n')
	findimg_file.write(' alpha 1.439515e+00 6.317441e-02 6.092828e-02 1.385599e-01 4.678523e+01 1.878385e-02 -2.058010e+01 0.0 0.0 1.000000e+00\n')
	...
	findimg_out = commands.getstatusoutput('./lensmodel 173853_findimg.input')
	...
	if(quad_imghead[0] == '#'):
		print quad_imghead
		raytrace_out = open('../../data/sub_gravlens/snap99_173853/sie_p1_src'+str(i)+'.txt','w')
		...
		find_table = np.loadtxt('../../data/sub_gravlens/snap99_173853/sie_p1_src'+str(i)+'.txt',skiprows=1)
