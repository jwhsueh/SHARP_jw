...
src_file = '/Volumes/sting_1/snap99_177217/177217_p3_src.dat'
src_rad = np.loadtxt(src_file)
src_px,src_py= np.degrees(src_rad[:,0])*3600,np.degrees(src_rad[:,1])*3600
src_g0 = np.array([ -4.323723e-02,  8.281401e-02])
src_p0 = np.array([src_px[0],src_py[0]])

src_tx,src_ty = (src_px-src_p0[0])+src_g0[0], (src_py-src_p0[1])+src_g0[1]

# SIE mag output
sie_out = open('/Volumes/sting_1/data/Rcusp_c/177217_p3_64_Rcusp_sie.txt','w')

for i in range(src_tx.size):
	findimg_file = open('177217_findimg.input','w')
	print src_tx[i],src_ty[i]
	findimg_file.write('gridmode 1\nset ngrid1=25\nset ngrid2=25\nset maxlev=3\n')
	findimg_file.write('startup 1 1\n')
	findimg_file.write('   alpha 1.163484e+00 -2.176496e-02 -2.066722e-03 1.698636e-01 5.155515e+00 3.935451e-02 5.910365e+01 0.0 0.0 1.000000e+00\n')
	...
	findimg_out = commands.getstatusoutput('./lensmodel 177217_findimg.input')
	...
	if(quad_imghead[0] == '#'):
		print quad_imghead
		raytrace_out = open('../../data/sub_gravlens/snap99_177217/sie_p3_src'+str(i)+'.txt','w')
		...
		find_table = np.loadtxt('../../data/sub_gravlens/snap99_177217/sie_p3_src'+str(i)+'.txt',skiprows=1)
